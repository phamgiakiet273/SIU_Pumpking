...
os.environ['CUDA_VISIBLE_DEVICES'] = SIGLIPV2BetaConfig().SIGLIP_V2_CUDA_VISIBLE_DEVICES

class SIGLIPV2BetaHandler:
    def __init__(
        self,
    ...
    def setup_database_handler(self) -> APIResponse:
        ...
        st = time.time()
        self.qdrant.addDatabase(collection_name = SIGLIPV2BetaConfig().SIGLIP_V2_DATABASE_NAME,
                                feature_size = int(SIGLIPV2BetaConfig().SIGLIP_V2_FEATURES_SIZE),
                                KEYFRAME_FOLDER_PATH = AppConfig().KEYFRAME_FOLDER_PATH,
                                FEATURES_PATH = SIGLIPV2BetaConfig().SIGLIP_V2_FEATURES_PATH,
        ...
        dummy_query = np.load(SIGLIPV2BetaConfig().SIGLIP_V2_DUMMY_VECTOR_PATH).reshape(1,-1).astype('float32')[0]
