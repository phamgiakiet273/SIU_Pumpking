...
class METACLIPHandler:
    def __init__(self, qdrant_database: QDRANT, model: METACLIP) -> None:
        logger.info("Initialized GeneralHandler with QDRANT and METACLIP model")
    ...
    def setup_database_handler(self) -> APIResponse:
        ...
        self.qdrant.addDatabase(
            collection_name=METACLIPConfig().METACLIP_DATABASE_NAME,
            feature_size=int(METACLIPConfig().METACLIP_FEATURES_SIZE),
            KEYFRAME_FOLDER_PATH=AppConfig().KEYFRAME_FOLDER_PATH,
            FEATURES_PATH=METACLIPConfig().METACLIP_FEATURES_PATH,
        ...
        dummy_query = (
            np.load(METACLIPConfig().METACLIP_DUMMY_VECTOR_PATH)
            .reshape(1, -1)
            .astype("float32")[0]
        )
