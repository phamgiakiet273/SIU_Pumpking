...
class SIGLIPV2BetaConfig:
    def __init__(self) -> None:
        ...
        features_path_env = os.getenv("SIGLIP_V2_B_FEATURES_PATH")
        assert features_path_env, "Environment variable 'SIGLIP_V2_B_FEATURES_PATH' must be set"
        try:
            self.SIGLIP_V2_FEATURES_PATH: List[str] = ast.literal_eval(features_path_env)
        except Exception as e:
            raise ValueError(f"Invalid list format for SIGLIP_V2_B_FEATURES_PATH: {features_path_env}") from e

        for path in self.SIGLIP_V2_FEATURES_PATH:
            if not os.path.exists(path):
                raise FileNotFoundError(f"Feature path '{path}' does not exist")

        self.SIGLIP_V2_FEATURES_SIZE = int(os.getenv("SIGLIP_V2_B_FEATURES_SIZE", "1536"))

        self.SIGLIP_V2_DUMMY_VECTOR_PATH = os.getenv("SIGLIP_V2_B_DUMMY_VECTOR_PATH")
        assert self.SIGLIP_V2_DUMMY_VECTOR_PATH, "Environment variable 'SIGLIP_V2_B_DUMMY_VECTOR_PATH' must be set"
